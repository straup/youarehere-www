...
m()}if(a)if(a.geometry.type=="Point"&&a.properties)$("#iamhere-"+(a.id?a.id:a.properties.id)).attr("class","info");else{if(a.properties){(c=a.properties.label)||(c=a.properties.name);c&&k(c)}if(a.properties)if($("#whereami-places")){(c=a.properties.woe_id)||(c=a.properties.id);c&&$("#woe-"+c).attr("selected","selected")}}},h=function(a){a=a.target;var c=a.feature;d.resetStyle(a);m();k(null);if(c&&c.geometry.type=="Point"&&c.properties)$("#iamhere-"+(c.id?c.id:c.properties.id)).removeAttr("class")},
i=function(a){a=a.target.feature;if(a.properties){a=a.properties;if(a.permalink)location.href=a.permalink}};d=L.geoJson(b,{style:function(a){if(a.geometry.type!="Point")return f},pointToLayer:function(a,c){return L.circleMarker(c,j)},onEachFeature:function(a,c){c.on({click:i,mouseover:l,mouseout:h})}});d.addTo(e);b.features[0].geometry.type=="Point"&&_points.push(d);var m=function(){for(var a=_points.length,c=0;c<a;c++)_points[c].bringToFront()};return d}}

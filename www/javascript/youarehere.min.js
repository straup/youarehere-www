function youarehere_locate(){var a=function(d){var e=$("#status");e.html("<li>"+d+"</li>"+e.html())};navigator.geolocation.getCurrentPosition(function(d){d=d.coords;var e=d.latitude,g=d.longitude;a("The sensors have placed you at or around "+e+","+g+".");a("Now to figure out where that is.");setTimeout(function(){location.href=location.href+"?lat="+e+"&lon="+g},2500)},function(){});a("Asking the sky where you are.")};function youarehere_correction_draw_map(a){$.ajax({url:a,success:function(d){youarehere_map_init(d);youarehere_woe_draw_shapes([d.features[0].properties.woe_id])}})};var _map=null,_points=[];function youarehere_map(){if(!_map){$("#".container_id).show();var a=L.map("map",{zoomControl:false,attributionControl:false});a.on("move",youarehere_map_coords);L.tileLayer("http://tile.stamen.com/toner-background/{z}/{x}/{y}.jpg",{attribution:"",maxZoom:18,minZoom:1}).addTo(a);_map=a}return _map}function youarehere_map_init(a){youarehere_map();youarehere_map_set_viewport(a);youarehere_map_draw_features(a)}
...
function youarehere_map_coords(){if($("#getlatlon-crosshairs")){var a=youarehere_map(),d=a.getCenter();a=a.getZoom();var e=d.lat;d=d.lng;var g="/maybe?lat="+e+"&lon="+d+"&zoom="+a,h="The crosshairs are pointing to ";h+=e.toFixed(6)+", "+d.toFixed(6);h+=" @ zoom level "+a;h+=' <span class="pointer">\u21e6</span> <a href="'+g+'">Try here instead</a>';$("#map-coords").html(h)}};function youarehere_woe_draw_shapes(a){for(var d=undefined,e=undefined,g=undefined,h=undefined,l=a.length,k=[],i=0;i<l;i++){var j=a[i];j!=-1&&$.inArray(j,k)==-1&&k.push(j)}l=k.length;for(i=0;i<l;i++){j=k[i];a=_cfg.woedb_static_url_template.replace("{W}",j);var m=function(c){youarehere_map_update_feedback("drawing place boundaries...");var b=undefined;if(c.type=="FeatureCollection"){b=c;c=b.features[0];if(!c.bbox){var f=c.geometry.coordinates;f=f[0][0];f=youarehere_map_coords_to_bbox(f,"lonlat");c.bbox=
